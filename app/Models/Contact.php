...
class Contact extends VCardResource
{
    ...
    public function scopeHasAllLabels($query, array $labelIds)
    {
        // Return all contacts if no labels specified
        if (empty($labelIds)) {
            return $query;
        }

        // Count how many labels we need to match
        $count = count($labelIds);

        // Single SQL subquery — AND logic (contact must have ALL specified labels)
        return $query->whereRaw(
            "(
            SELECT COUNT(DISTINCT label_id)
            FROM contact_label
            WHERE contact_id = contacts.id
            AND label_id IN (" . implode(',', array_fill(0, $count, '?')) . ")
        ) = ?",
            array_merge($labelIds, [$count])
        );
    }
